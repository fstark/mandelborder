...
MandelbrotApp::MandelbrotApp(int w, int h, bool speed,
    ...
    : width(w), height(h), pixelSize(1), window(nullptr), renderer(nullptr),
      texture(nullptr), glContext(nullptr), ownsGLContext(false),
      autoZoomActive(false), speedMode(speed), verboseMode(false),
      exitAfterFirstDisplay(false), autoScreenshotMode(false),
  ...
  if (engineType == "border") {
    ...
    std::cerr << "Unknown engine type: " << engineType
              << ", defaulting to BORDER" << std::endl;
    currentEngineType = GridMandelbrotCalculator::EngineType::BORDER;
  }
...
#ifdef __APPLE__
  // On macOS, the renderer uses Metal, so we need to create an OpenGL context
  // explicitly
  glContext = SDL_GL_CreateContext(window);
  if (!glContext) {
    throw std::runtime_error(std::string("OpenGL context creation failed: ") +
                             SDL_GetError());
  }
  ownsGLContext = true;
  ...
  // Create renderer first (it will create its own OpenGL context if
  // accelerated)
  ...
  if (!texture) {
    throw std::runtime_error(std::string("Texture creation failed: ") +
                             SDL_GetError());
  }

  // Speed mode: 4x4 grid with parallel computation
  // Normal mode: 1x1 grid (effectively single calculator) with progressive
  // rendering
  if (currentEngineType == GridMandelbrotCalculator::EngineType::GPUF ||
      currentEngineType == GridMandelbrotCalculator::EngineType::GPUD) {
    // GPU always uses 1x1 grid
    auto gridCalc =
        std::make_unique<GridMandelbrotCalculator>(calcWidth, calcHeight, 1, 1);
    gridCalc->setSpeedMode(speedMode);
    gridCalc->setEngineType(currentEngineType);
    calculator = std::move(gridCalc);
  } else if (speedMode) {
    auto gridCalc =
        std::make_unique<GridMandelbrotCalculator>(calcWidth, calcHeight, 4, 4);
  ...
  // Use fixed polynomial: r(t) = 9*(1-t)*t³*255, g(t) = 15*(1-t)²*t²*255, b(t)
  // = 8.5*(1-t)³*t*255
...
void MandelbrotApp::initSDL() {
  ...
  window = SDL_CreateWindow(
      "Mandelbrot Set - Boundary Tracing", SDL_WINDOWPOS_CENTERED,
...
// Removed switchToOpenGL and switchToSDLRenderer as we now use a unified
// approach
...
void MandelbrotApp::compute() {
  ...
  if (verboseMode) {
    ...
    std::cout << std::format(
        "{} {:>4}x{:<4} {:>8.1f} ms  {:>20.16f} {:>20.16f} {:>12.2e}\n",
        engineName, calculator->getWidth(), calculator->getHeight(),
        milliseconds, calculator->getCre(), calculator->getCim(),
        calculator->getDiam());
  }
}
...
SDL_Rect MandelbrotApp::calculateSelectionRect(int startX, int startY, int endX,
                                               int endY, bool centerBased) {
  int dx = endX - startX;
  int dy = endY - startY;
...
void MandelbrotApp::resetZoom() { calculator->updateBounds(-0.5, 0.0, 3.0); }
...
void MandelbrotApp::animateRectToRect(int startX, int startY, int startWidth,
                                      int startHeight, int endX, int endY,
                                      int endWidth, int endHeight, int steps,
                                      int frameDelay) {
...
void MandelbrotApp::blinkRect(int x, int y, int w, int h, int times,
                              int blinkDelay) {
...
void MandelbrotApp::zoomToRect(int x1, int y1, int x2, int y2, bool inverse) {
  ...
  if (inverse) {
    ...
    double scale =
        std::max((double)width / (x2 - x1), (double)height / (y2 - y1));
    int offsetX = (x1 + x2) / 2 - width / 2;
    int offsetY = (y1 + y2) / 2 - height / 2;

    // Adjust step size for resolution difference
    double effectiveStepR =
        calculator->getStepR() * ((double)calcWidth / width);
    double effectiveStepI =
        calculator->getStepI() * ((double)calcHeight / height);
...
void MandelbrotApp::run() {
  if (!exitAfterFirstDisplay) {
    ...
    std::cout << "  F        - Toggle fast mode (parallel computation)"
              << std::endl;
    std::cout << "  S        - Save screenshot" << std::endl;
    std::cout << "  Shift+S  - Toggle auto-screenshot mode" << std::endl;
    std::cout << "  E        - Cycle engine "
                 "(Border→Standard→SIMD→GPU-Float→GPU-Double)"
              << std::endl;
  ...
  while (running) {
      ...
      case SDL_KEYUP:
        if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_ESCAPE) {
          ...
          if (modState & KMOD_SHIFT) {
            ...
            } // If we just enabled auto-screenshot, capture the current frame
              // immediately
            if (autoScreenshotMode) {
              saveScreenshot("mandelbrot");
            }
          ...
          if (currentEngineType == GridMandelbrotCalculator::EngineType::GPUF ||
              currentEngineType == GridMandelbrotCalculator::EngineType::GPUD) {
            auto gridCalc = std::make_unique<GridMandelbrotCalculator>(
                calcWidth, calcHeight, 1, 1);
            gridCalc->setSpeedMode(speedMode);
            gridCalc->setEngineType(currentEngineType);
            calculator = std::move(gridCalc);
            std::cout << "Speed mode: " << (speedMode ? "ON" : "OFF")
                      << " (GPU 1x1)" << std::endl;
          } else if (speedMode) {
            auto gridCalc = std::make_unique<GridMandelbrotCalculator>(
                calcWidth, calcHeight, 4, 4);
            gridCalc->setSpeedMode(true);
            gridCalc->setEngineType(currentEngineType);
            calculator = std::move(gridCalc);
          } else {
          ...
          if (currentEngineType ==
            ...
            currentEngineType = GridMandelbrotCalculator::EngineType::GPUF;
                     ...
                     GridMandelbrotCalculator::EngineType::GPUF) {
            currentEngineType = GridMandelbrotCalculator::EngineType::GPUD;
          } else {
            currentEngineType = GridMandelbrotCalculator::EngineType::BORDER;
          }
          ...
          if (currentEngineType == GridMandelbrotCalculator::EngineType::GPUF ||
            ...
            auto gridCalc = std::make_unique<GridMandelbrotCalculator>(
                calcWidth, calcHeight, rows, cols);
            gridCalc->setSpeedMode(speedMode);
            gridCalc->setEngineType(currentEngineType);
            calculator = std::move(gridCalc);
          }
          ...
          SDL_Rect rect =
              calculateSelectionRect(dragStartX, dragStartY, currentMouseX,
                                     currentMouseY, ctrlPressed);
          SDL_RenderDrawRect(renderer, &rect);
          SDL_RenderPresent(renderer);
        }
        break;
      ...
      case SDL_MOUSEBUTTONUP:
        if (event.button.button == SDL_BUTTON_LEFT && dragging) {
          ...
          if (!shiftPressed && isZoomDisabled()) {
            std::cout << "Zoom disabled: diameter too small ("
                      << calculator->getDiam() << ")" << std::endl;
            break;
          }
          ...
          if (dragDistance < 5) {
            ...
            SDL_Rect rect = calculateSelectionRect(
                dragStartX, dragStartY, dragEndX, dragEndY, ctrlPressed);
            x1 = rect.x;
            y1 = rect.y;
            x2 = x1 + rect.w;
            y2 = y1 + rect.h;
    ...
    if (autoZoomActive) {
      ...
      if (isZoomDisabled()) {
        ...
        zoomChooser->findInterestingPoint(
            calculator->getData(), MandelbrotCalculator::MAX_ITER, calcCenterX,
            calcCenterY, calcRectW, calcRectH);
...
std::string
MandelbrotApp::generateUniqueFilename(const std::string &basename,
  ...
  for (int i = 0; i <= 999; ++i) {
    ...
    oss << baseFilename << "-" << std::setfill('0') << std::setw(3) << i
        << extension;
  ...
  throw std::runtime_error(
      "Cannot generate unique filename: too many files with timestamp " +
...
void MandelbrotApp::saveScreenshot(const std::string &basename) {
  ...
  try {
    ...
    std::cerr << "Failed to generate unique filename: " << e.what()
              << std::endl;
    return;
  }
  ...
  if (!SDL_RenderReadPixels(renderer, NULL, SDL_PIXELFORMAT_RGB24, rgbPixels.data(),
                           3 * width)) {
    // Pitch for RGB is simply width * 3
    int result = stbi_write_png(filename.c_str(), width, height, 3,
                                rgbPixels.data(), width * 3);
    ...
    std::cerr << "Failed to retrieve screenshot: " << filename << std::endl;
  }
}
...
void MandelbrotApp::setAutoZoom(bool enabled) { autoZoomActive = enabled; }

void MandelbrotApp::setRandomPalette() { gradient = Gradient::createRandom(); }
